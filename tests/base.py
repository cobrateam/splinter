...
class WebDriverTests(BaseBrowserTests):
    ...
    def test_find_by_css_selector_using_a_custom_wait_time_should_found_an_async_element(self):
        "should find by css selector using a custom wait time found an async element" 
        self.browser.find_by_css_selector('.add-async-element').first.click()
        assert 1 == len(self.browser.find_by_css_selector('.async-element2', wait_time=3))
    ...
    def test_find_by_id_should_found_an_async_element(self):
        ...
        self.browser.find_by_css_selector('.add-async-element').first.click()
        assert 1 == len(self.browser.find_by_id('async-header', wait_time=10))
